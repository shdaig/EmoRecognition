...
with FaceLandmarker.create_from_options(options) as landmarker:
    for frame_1_filename in neutral_frames_path_1:
        ...
        if len(face_landmarker_result_1.face_blendshapes) != 0 and len(face_landmarker_result_2.face_blendshapes) != 0:
            face_blendshapes_scores_1 = get_face_blendshapes_scores(face_landmarker_result_1.face_blendshapes[0])
            face_blendshapes_scores_2 = get_face_blendshapes_scores(face_landmarker_result_2.face_blendshapes[0])
            for i in range(len(face_blendshapes_names)):
                blendshapes_dict[face_blendshapes_names[i]].append(face_blendshapes_scores_2[i] - face_blendshapes_scores_1[i])
...
blendshapes_df.to_csv("frames_diff_blendshapes.csv", index=False)
