...
def qual_plot_data(fname, raw=None, force=True):
    ...
    if os.path.exists(cache) and (not force):
        ...
        for i, lag_time in enumerate(lag_times):
            for j, lag_time2 in enumerate(lag_times2):
                if abs(lag_time - lag_time2) <= 10:
                    bad_lags2_idxs.append(j)
        for i in range(len(lag_times2)):
            if i not in bad_lags2_idxs:
                new_lags2.append(lags2[i])
                new_lag_times2.append(lag_times2[i])
        
        lags2 = np.array(new_lags2)
        lag_times2 = np.array(new_lag_times2)
        # ****************************************************************
...
def plot_qual(lags, lag_times, lags2, lag_times2, first_mark_time, react_range, q, plot_IPE=True):
    # print(f"anton: {len(lags)}")
    fig = plt.figure(figsize=(24, 4))
    ax = plt.subplot(111)
    ax.scatter(lag_times+first_mark_time, lags, c='g', alpha=.7, s=30)
    ax.set_ylabel('Reaction lag, sec')
    ax.set_xlabel('Time, sec')
    # ax.scatter(lag_times2+first_mark_time, lags2, marker='x', c='r', alpha=.5, s=30)
    ax.scatter(lag_times2+first_mark_time, lags2, marker='x', c='r', alpha=.7, s=30)
    ax = ax.twinx()
    if plot_IPE:
...
def plot_qual_ax(lags, lag_times, lags2, lag_times2, first_mark_time, react_range, q, ax, plot_IPE=True):
    # print(f"anton: {len(lags)}")
    # fig = plt.figure(figsize=(24, 4))
    # ax = plt.subplot(111)
    ax = ax.twinx()
